...
const MtechGraduated = () => {
  ...
  // Fetch all graduated M.Tech students
  const fetchData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const mtechData = data.filter((student) => student.enrolledIn === "M.Tech");
        setStudentsData(mtechData);
        if (mtechData.length === 0) {
          toast.info("No M.Tech graduates found");
        } else {
          toast.success(`Loaded ${mtechData.length} M.Tech graduate${mtechData.length > 1 ? 's' : ''}`);
        }
      } else {
  ...
  const handleOpenDialog = (mode, student = null) => {
    ...
    if (mode === "edit" && student) {
      setFormData({
        name: student.name || "",
        enrolledIn: student.enrolledIn || "M.Tech",
    ...
    <Box sx={styles.container}>
      ...
      <Box sx={styles.header}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {filteredData.length} M.Tech graduate(s) found
          </Typography>
        </Box>
        <Box>
          <Chip
      ...
      <Paper sx={styles.actionBar}>
        <Grid container spacing={2} alignItems="center">
          <Grid item xs={12} md={6}>
            <TextField
              fullWidth
              placeholder="Search by name, thesis, branch, or workplace..."
              value={searchQuery}
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Paper sx={styles.statCard}>
            <Box sx={styles.statCardContent}>
              <WorkspacePremiumIcon sx={{ fontSize: 40, color: "warning.main", mb: 1 }} />
              <Typography variant="h4">{recentGraduates}</Typography>
              <Typography variant="body2" color="text.secondary">
                Recent Graduates (5 yrs)
              </Typography>
            </Box>
          </Paper>
        </Grid>
          ...
          <Box sx={styles.noDataContainer}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
              {searchQuery
                ...
                : "No M.Tech graduates available. Add your first graduate!"}
            </Typography>
            <Button
              variant="contained"
              startIcon={<AddIcon />}
      ...
      <Dialog open={dialogOpen} onClose={handleCloseDialog} maxWidth="md" fullWidth>
        <DialogTitle>
          {dialogMode === "add" && "Add New M.Tech Graduate"}
          {dialogMode === "edit" && "Edit M.Tech Graduate"}
          {dialogMode === "delete" && "Delete M.Tech Graduate"}
        </DialogTitle>
        <form onSubmit={handleSubmit}>
          ...
          <DialogActions>
            ...
            >
              {dialogMode === "add" && "Add Graduate"}
              {dialogMode === "edit" && "Update Graduate"}
              {dialogMode === "delete" && "Delete Graduate"}
            </Button>
          </DialogActions>
        </form>
      </Dialog>
